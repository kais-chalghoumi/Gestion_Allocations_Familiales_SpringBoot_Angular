...
export class ListBeneficiaireComponent implements OnInit {
  ...
  search(assMat: number, assCle: number) {
    if (assMat == null || assCle == null) {
      this.errorMessage = 'الرجاء إدخال رقم المضمون الاجتماعي و المفتاح';
              ...
              .subscribe((res: VueAF[]) => {
                ...
                this.listVueAf.forEach((element) => {
                  if (element.docBen == '2') {
                    ...
                    element.docBen =
                      'شهادة طبية للطفل المصاب بعجز أو بداء عضال.';
  ...
  openModal(benIduCnss: any) {
    ...
    this.services
      .getAge(benIduCnss)
      .pipe(
        switchMap((age: number) => {
          this.age = age;
          console.log(this.age);
          return this.services.getAllDocumentLibellesByAge(this.age);
        })
      )
